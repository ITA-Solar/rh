...
/*	atom->C[ji][k] += cup; */
  ...
  /* ---- Graham Kerr, Sept 15 2021                    -------------- */
  /*      Adding non-thermal collisional ionisation and excitation for 
          hydrogen and helium.
  */
  ...
  //      printf("\n\n>>> Including Non-Thermal Collisional Rates for %2s\n",atom->ID);
  //      // printf("\n(Num of levels: %d\n)",atom->Nlevel);
  //      // readCnt_hdf5(mpi.xnum[mpi.ix],mpi.ynum[mpi.iy], &atmos, &geometry,
  //      //              &infile, atom, atom->offset_coll);
  //      readCnt_hdf5(mpi.xnum[mpi.ix],mpi.ynum[mpi.iy], &atmos, &geometry,
  //                   &infile, atom);
  //     }
  ...
  if (strcmp(atom->ID,"HE") == 0){
       ...
       i = 0;
       ji = j*atom->Nlevel + i;
       printf("\n\n>>> CIJ therm collision_p = %.17f",atom->C[ji][k]);
