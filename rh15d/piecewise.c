...
/* JdlCR: This implementation has the wrong coefficients, probably
   because it re-uses a calculation for parabolic interpolation
   --- */
		  ...
		  double *chi, double *S, double *I, double *Psi)
{
  ...
  for (k = k_start+dk;  k != k_end+dk;  k += dk) {
    ...
    /* ---
       Calculate coeffs for formal solution:
