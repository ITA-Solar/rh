...
double E1(double x)
{
  ...
  if (x <= 0.0) {
    sprintf(messageStr, "Irrecoverable: Exponential integral E1 of x = %e", x);
    Error(WARNING, "E1", messageStr);   /* Only warning because of mpi.stop */
    mpi.stop = TRUE;
    return 0.0;
  }
  else if (x > 0.0  &&  x <= 1.0) {
...
double E2(double x)
{
  ...
  if (x <= 0.0) {
    sprintf(messageStr, "Irrecoverable: Exponential integral E2 of x = %e", x);
    Error(WARNING, "E2", messageStr);   /* Only warning because of mpi.stop */
    mpi.stop = TRUE;
    return 0.0;
